...
export default function LoginScreen() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (session) {
        router.replace("/(tabs)"); // Agar login hai to seedha Home
      }
    };
  ...
  const handleAuth = async () => {
    ...
    try {
      if (isSignUp) {
        ...
        // Supabase automatic email confirmation check karta hai default par
        if (data.session) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.formContainer}>
          ...
          {loading ? (
            ...
            <Text style={styles.authButtonText}>{isSignUp ? 'Sign Up 🎉' : 'Login 🏃‍♂️'}</Text>
          )}
        </TouchableOpacity> */}
          <CustomButton
            // title="Continue with Email ✉️"
