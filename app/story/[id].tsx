...
export default function StoryDetail() {
  ...
  useEffect(() => {
    Speech.stop();
    stopHighlightTimer();
    setCurrentWordIndex(-1); // Reset highlight on slide change
  ...
  // ── HIGHLIGHT TIMING CONTROLS ──────────────────────────────────
  const startHighlightTimer = (text: string, rate: number) => {
    stopHighlightTimer();
    const words = text.split(" ");
    if (words.length === 0) return;

    // Estimate speed: standard rate per word based on current TTS rate
    const baseMsPerWord = lang === "ur" ? 480 : 400;
  ...
  const playVoiceOver = () => {
    ...
    // Voice start hone se pehle timer shuru karein
    startHighlightTimer(textToSpeak, currentRate);
      ...
      <Text style={styles.storyText}>
        {words.map((word: string, index: number) => {
          ...
          return (
            <Text
              key={index}
              style={isHighlighted ? styles.highlightedWord : styles.normalWord}
            >
              {word}{" "}
            </Text>
          );
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <View style={styles.textBackground}>
          {/* FIXED: Single words are highlighted inside this function */}
